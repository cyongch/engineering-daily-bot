...
const BID_DROP = /(预中标|中标人|评标结果|开标结果|中标价|投标人|投标报价|招标公告|招标公示|招标信息|中标公告|中标公示|中标结果|中标候选人|成交公告|成交结果|采购公告|采购结果|询价公告|询比|询价采购|竞争性谈判|竞争性磋商|竞价公告|资格预审|开标|评标|定标|废标|流标|邀标|比选公告|采购服务|供应商|采购项目|招标项目|发包公告|招标预告|拟建项目|在建项目|中标|招投标)/;
  ...
  function pickCandidates(src, html) {
    ...
    const arr = extract(src.url, html, src.kw || KW)
      .filter(a => {
        ...
        if (BID_DROP.test(a.title.replace(/[（(][^）)\n]*[）)]/g, ''))) return false; // 剔除括号干扰后判招标/中标/采购公告
